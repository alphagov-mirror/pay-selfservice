...
module.exports = (req, res) => {
  ...
  if (updatedPageData.productNamePath === '') {
    req.flash('genericErrorOld', `<h2>There was a problem with the details you gave for:</h2><ul class="govuk-list govuk-error-summary__list"><li><a href="#payment-name-path">Please change the website address</a></li></ul>`)
  ...
  productsClient.product.getByProductPath(updatedPageData.serviceNamePath, updatedPageData.productNamePath)
    ...
    // if product exists we need to alert the user they must use a different URL
      req.flash('genericErrorOld', `<h2>There was a problem with the details you gave for:</h2><ul class="govuk-list govuk-error-summary__list"><li><a href="#payment-name-path">Website address</a></li></ul>`)
      return res.redirect(paths.paymentLinks.webAddress)
    })
